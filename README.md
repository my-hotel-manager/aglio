### This is a fork of Aglio that uses drafter.js instead of drafter.

This reduces the build time massively, which is useful when deploying a project. The drawback is that API Blueprint files are parsed much slower and is only recommended if used a build step to serve the generated HTML file.
...
```javascript
aglio.renderFile("/tmp/input.apib", "/tmp/output.html", options, function (
  err,
  warnings
) {
  if (err) return console.log(err);
